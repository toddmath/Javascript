...
function rotateLeft(bits: string, turns: number): string {
	return bits.slice(turns) + bits.slice(0, Math.max(0, turns))
}
...
export function SHA1(message: string): string {
	...
	chunks.forEach(function (chunk) {
		...
		for (let i = 0; i < 80; i++) {
			...
			const aInt = Number.parseInt(aRot, 2) >>> 0
			const wordInt = Number.parseInt(words[i], 2) >>> 0
			const t = aInt + f + e + k + wordInt
			e = d >>> 0
			d = c >>> 0
			const bRot = rotateLeft(pad(b.toString(2), 32), 30)
			c = Number.parseInt(bRot, 2) >>> 0
			b = a >>> 0
			a = t >>> 0
		}
