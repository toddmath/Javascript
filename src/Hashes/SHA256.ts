 ...
 * @param {number} bits - total number of bits wanted
 ...
 * @param {number} size - number of characters wanted in each chunk
 * @return {Array<string>} - array of original string split into chunks
 * @example
 * ```ts
 ...
 * ```
 */
function rotateRight(bits: string, turns: number): string {
	return (
		bits.slice(bits.length - turns) +
		bits.slice(0, Math.max(0, bits.length - turns))
	)
}
...
function preProcess(message: string): string {
	...
	let m =
		message
			.split('')
			.map((e) => pad(e.charCodeAt(0).toString(2), 8))
			.join('') + '1'
