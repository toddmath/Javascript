 ...
 ******************************************************/
export function keyFinder(str: string | number): number {
...
function caesarCipherEncodeAndDecodeEngine(
	inStr: string,
	numShifted: number
): string {
	...
	for (let i = 0; i < inStr.length; i++) {
		// ? Added 0 to charCodeAt ??
		charCode = inStr[i].charCodeAt(0)
		...
		if (charCode >= 48 && charCode <= 57) {
			if (shiftedCharCode < 48) {
				...
				diffElem.innerHTML = diff.toString()
		...
		const finalResult = typeof result === 'number' ? result : parseInt(result)
		outStr = outStr + String.fromCharCode(finalResult)
	}
